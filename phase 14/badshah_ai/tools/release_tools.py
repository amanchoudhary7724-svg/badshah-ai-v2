...
def export_workspace():
    ...
    zip_dir(settings.safe_workspace, out)
    return f"Workspace exported: {out}"

def release_package():
    out = settings.export_dir / f"badshah_ai_v1_4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
    zip_dir(BASE_DIR, out)
