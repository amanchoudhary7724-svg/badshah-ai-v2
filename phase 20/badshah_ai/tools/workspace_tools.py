...
def safe_path(name):
    ...
    if not str(target).startswith(str(base)): raise ValueError("Unsafe path blocked")
    return target
...
def create_website(name="portfolio"):
    ...
    write_file(f"{name}/index.html", "<h1>BADSHAH Website</h1><p>Multi-Agent Planner Ready.</p>")
