    ...
    def run(self, q: str) -> str:
        ...
        try:
            if x == "help":
                ...
                ans = f"BADSHAH-AI v{badshah_ai.__version__}"
            elif "health" in x:
                ans = self.llm.health()
            elif "validate env" in x:
                ans = "ENV OK" if settings.ollama_url.startswith("http") else "ENV issue"
            elif x == "plugins":
                ans = json.dumps(list_plugins(), indent=2)
            elif x == "memory":
                rows = self.store.recent_memory()
                ans = "\n".join([f"{t} | {qq} => {r[:80]}" for qq,r,t in rows]) or "No memory yet."
            elif x == "tasks":
                rows = self.store.recent_tasks()
                ans = "\n".join([f"{t} | {s} | {qq}" for qq,s,r,t in rows]) or "No tasks yet."
            elif x.startswith("browser title"):
                ans = browser_title(self._extract_url(q))
            elif x.startswith("browser text"):
                ans = browser_text(self._extract_url(q))
            elif x.startswith("browser screenshot"):
                ans = browser_screenshot(self._extract_url(q))
                ...
                ans = write_file(fn, content)
            elif x.startswith("read file "):
                ans = read_file(q.split(" ", 2)[2])
            elif x.startswith("pdf ") or ".pdf" in x:
                ans = pdf_text(self._extract_path(q, ["pdf"]))
            elif x.startswith("excel ") or ".xlsx" in x or ".xls" in x or ".csv" in x:
                ans = excel_summary(self._extract_path(q, ["xlsx","xls","csv"]))
            elif x.startswith("ocr ") or any(e in x for e in [".png",".jpg",".jpeg",".webp",".bmp"]):
                ans = ocr_image(self._extract_path(q, ["png","jpg","jpeg","webp","bmp"]), settings.tesseract_cmd)
            elif x.startswith("search "):
                ans = search_web(q[7:].strip())
            elif x.startswith("open app "):
                ans = open_app(q.replace("open app","").strip())
            elif x.startswith("open "):
                ans = open_url(q.split(" ",1)[1])
            elif "export workspace" in x:
                ans = export_workspace()
            elif "release package" in x:
                ans = release_package()
            else:
                ans = self.llm.generate(q)
            self.store.add_memory(q, ans)
            self.store.add_task(q, "success", ans[:1000])
            return ans
        except Exception as e:
            self.store.add_task(q, "error", str(e))
            return "Error: " + str(e)
