...
from badshah_ai.tools.screen_vision_tools import screen_safety, take_screenshot, image_ocr
...
class Brain:
    ...
    def execute_single(self, q):
        ...
        if x.startswith("plugin disable "): return self.plugins.disable(q.split(" ", 2)[2])
        if x == "contacts": return list_contacts()
        if x.startswith("add contact "):
            parts = q.split(" ", 3)
            return add_contact(parts[2], parts[3]) if len(parts) >= 4 else "Usage: add contact NAME VALUE"
        ...
        if x.startswith("image ocr "): return image_ocr(q.split(" ", 2)[2])
        ...
        if x == "build exe guide": return "Run installer\\BUILD_EXE.bat"
