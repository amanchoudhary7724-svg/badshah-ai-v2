...
st.title("BADSHAH-AI v2 Phase 7 — Plugin Ready")
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Chat","Self Modify","Plugins","Tasks","Settings"])
...
with tab1:
    ...
    with col1:
        if st.button("Help"):
            st.code(brain.run("help"))
    with col2:
        if st.button("Diagnostics"):
            st.code(brain.run("diagnostics"))
    with col3:
