...
class HelpAgent(BaseAgent):
    name = "help"
    def handle(self,q): return HELP_TEXT

class ConfigAgent(BaseAgent):
    name = "config"
    def handle(self,q): return json.dumps(settings.as_dict(), indent=2)

class PluginAgent(BaseAgent):
    name = "plugins"
    def handle(self,q): return json.dumps(list_plugins(), indent=2)

class DiagnosticsAgent(BaseAgent):
    name = "diagnostics"
    def handle(self,q): return diagnostics()
