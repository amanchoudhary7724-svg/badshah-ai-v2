...
class MemoryEngine:
    ...
    def _init_sqlite(self):
        with sqlite3.connect(self.db) as con:
            con.execute(
                "CREATE TABLE IF NOT EXISTS memories(id INTEGER PRIMARY KEY, text TEXT, source TEXT, created_at TEXT)"
            )
    ...
    def remember(self, text, source="user"):
        ...
        if self.chroma:
            try:
                self.chroma.add(ids=[str(mid)], documents=[text], metadatas=[{"source": source, "created_at": now}])
            except Exception:
                pass
    ...
    def search(self, query, limit=5):
        if self.chroma:
            try:
                result = self.chroma.query(query_texts=[query], n_results=limit)
        ...
        with sqlite3.connect(self.db) as con:
            for w in words[:5] or [query]:
                rows += con.execute(
                    "SELECT text FROM memories WHERE lower(text) LIKE ? ORDER BY id DESC LIMIT ?",
                    (f"%{w}%", limit),
                ).fetchall()
        seen, out = set(), []
        for (text,) in rows:
    ...
    def clear(self):
        ...
        if self.chroma:
            try:
                # Recreate collection by deleting it.
                import chromadb
                client = chromadb.PersistentClient(path=str(settings.chroma_dir))
                try:
                    client.delete_collection("badshah_memory")
                except Exception:
                    pass
                self.chroma = client.get_or_create_collection("badshah_memory")
            except Exception:
