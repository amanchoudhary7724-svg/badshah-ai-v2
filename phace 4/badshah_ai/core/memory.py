...
class Memory:
    ...
    def store(self, query, response, tag="chat"):
        with sqlite3.connect(self.db) as con:
            con.execute("INSERT INTO memories(query,response,tag,created_at) VALUES(?,?,?,?)",(query,response,tag,datetime.utcnow().isoformat()))

    def recall(self, query, limit=5):
        word=(query.split() or [""])[0]
        with sqlite3.connect(self.db) as con:
            rows=con.execute("SELECT query,response FROM memories WHERE query LIKE ? OR response LIKE ? ORDER BY id DESC LIMIT ?",(f"%{word}%",f"%{word}%",limit)).fetchall()
