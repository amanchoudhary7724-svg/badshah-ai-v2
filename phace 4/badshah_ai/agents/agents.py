...
class CodingAgent(BaseAgent):
    ...
    def handle(self,q):
        ...
        if x.startswith("write file "):
            _,_,fn,content=q.split(" ",3); return write_text_file(fn,content)
        if x.startswith("read file "): return read_text_file(q.split(" ",2)[2])
        if "create website" in x or "make website" in x: return create_static_website(q.replace("create website","").replace("make website","").strip())
...
class BrowserAgent(BaseAgent):
    ...
    def handle(self,q):
        x=q.lower()
        if "scrape" in x:
            m=re.search(r'(https?://\S+|www\.\S+|[\w.-]+\.[a-z]{2,}\S*)',q); return scrape(m.group(1)) if m else "URL missing"
...
class PDFAgent(BaseAgent):
    name="pdf"
    def handle(self,q):
        m=re.search(r'(.+\.pdf)',q,re.I); return extract_pdf_text(m.group(1).strip().strip('"')) if m else "PDF path missing"

class ExcelAgent(BaseAgent):
    name="excel"
    def handle(self,q):
        m=re.search(r'(.+\.(xlsx|xls|csv))',q,re.I); return summarize_table(m.group(1).strip().strip('"')) if m else "Excel/CSV path missing"

class VisionAgent(BaseAgent):
    name="vision"
    def handle(self,q):
        m=re.search(r'(.+\.(png|jpg|jpeg|webp|bmp))',q,re.I); return extract_image_text(m.group(1).strip().strip('"')) if m else "Image path missing"
